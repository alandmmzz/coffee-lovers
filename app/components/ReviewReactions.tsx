...
export default function ReviewReactions({
  ...
  function startPress() {
    timerRef.current = setTimeout(() => setShowPicker(true), 450);
  }
      ...
      {(reactions.length > 0 || showPicker) && (
        <div className="flex items-center gap-1.5 flex-wrap mt-3 pt-3 border-t border-parchment-dim/15">
          {reactions.length > 0
            ...
            : showPicker && (
                <span className="font-mono text-[10px] text-parchment-dim/60">Reaccionar</span>
              )}
        </div>
      )}

      {showPicker && (
        <div className="absolute bottom-full left-5 mb-1.5 flex gap-1 bg-ink-soft border border-parchment-dim/25 rounded-full px-2.5 py-1.5 shadow-xl z-20">
